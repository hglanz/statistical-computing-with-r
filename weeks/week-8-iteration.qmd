...
argument specifies which variables we want to transform, and it has a `<tidy-select>` tag. This means that the syntax we use for `.cols` follows the 
...
### `r emo::ji("check")`: Check-in 8.1: Connecting `across()` with `pivot_wider()` and `pivot_longer()`

1. Fill in the code below to convert all numeric columns into character columns.
...
make_unit_string <- function(x, unit){
  str_c(x, unit)
}
...
3. Fill in the code below that accomplishes #2 using a `pivot_longer()` 
