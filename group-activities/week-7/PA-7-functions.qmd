...
<!-- The person who wins rock-paper-scissors starts as the Developer (typing and
...
Hint: **Do not** use a `for()` loop to do this! Accomplish this with the
`seq_along()` function, bracket subsetting (`[]`), and modulus arithmetic
(`%%`).
...
Hint: **Do not** use a `while()` loop to do this! Accomplish this with the
`cumsum()` function and bracketing.
