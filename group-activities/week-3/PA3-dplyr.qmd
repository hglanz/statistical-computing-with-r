...
**Step 6:** Create a new variable called `TOTAL_APPLY` which contains the 
number of students who apply to the university every year. (You can assume the
size of the first year class is 1/4 of the undergraduate population.)

```{r}
#| label: add-total-apply

colleges_clean6 <- colleges_clean5 |> 
    mutate(APPLY = ADM_RATE * (UGDS / 4))
```

**Step 7:** Create a new variable called `TOTAL_IN` which contains the total
amount of money made from tuition per year.

```{r}
#| label: add-total-tuition

colleges_clean7 <- colleges_clean6 |> 
    select(TOTAL_IN = UGDS x TUITIONFEE_IN)
```

<!-- Swap roles -- Developer becomes Coder, Coder becomes Developer! -->

**Step 8:** Remove every row with missing data.

```{r}
#| label: drop-na

colleges_clean8 <- colleges_clean7 |> 
  drop.na()
```
...
**Step 9:** Combine your (fixed) code chunks into a **single** code chunk that
