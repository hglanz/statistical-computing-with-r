...
```{r}
#| label: finding-missing-values-recoding

```
Hint 1: information in the top 6 rows of the excel sheet will help you answer
this question.

Hint 2: You will find the following functions helpful: `mutate()`, `across()` 
(you will need two of these), `na_if()`

Hint 3: When referring to one of the year variable names, you **must** put tick marks (above the tab key) around the name. Starting the name of a variable with a number is not commonly read as a variable name. E.g., to read the 1988 column through the 2018 column, use ``` `1988`:`2019` ```.


**3. Because characters were used to indicate missing values, all of the columns 1988 through 2019 were read in as characters. Change these columns to a numeric data type. Save these changes into an updated version of `military_clean`.**

```{r}
#| label: changing-columns-as-numeric
...
Hint 1: Join by different variables in `dataX` and `dataY`: `join_by(a == b)`
...
Hint 1: Make sure you change the plot title and axis labels to accurately
represent the plot.

Hint 2: You might also want to change the x-axis limits and the color of your
plots.

Hint 3: Place the `Year` variable on an axis that makes it easier to read the
labels!
