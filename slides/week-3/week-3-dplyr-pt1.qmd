...
+ Make sure you **cite** external sources.
  + E.g., many people used `sapply()` to get the variable data types -- this is
  fine, but where did you find this solution? It's not in the textbook or the 
  coursework, so that function requires a citation! 

## Citing Your Sources

::: {.small}
When you write code, you will **need** to reference function / package
...
::: {.small}
When you rely on external resources for an assignment in this course, 
**you must cite your sources**.

+ If you use **any** resources outside of the course text, the course slides,
and the posted cheatsheets, you **must include a citation**!
+ If you do not cite your sources that constitutes plagiarism. 
+ Any instances of plagiarism in a lab or challenge assignment removes your 
ability to include code from that assignment in your portfolio. 
:::
...
The `,` separating these two conditions is equivalent to using an `&`. 

Why not just use a `&`? Well, with a lot of `&`s your code can get hard to
read...
...
+ `case_when()`: creating a new variable with three or more levels 
  * Pssst, don't use nested `if_else()` functions.
...
::: {.incremental}
::: {.small}
- the first column (far left) column provides guidance on finding grouped summaries (`group_by()` + `summarize()`)
- the second column provides guidance on `filter()`ing values with logical comparisons
- the third column has advice on `select()`ing and `mutate()`ing variables
:::
:::
:::
:::
...
::: {.incremental}
::: {.small}
- the second column provides a list of summary functions you might like to use
with `summarize()`
:::
:::
:::
...
The partner whose birthday comes last starts as the Listener!

They need to do the following:

- Click on the PA2 link in Canvas 
- Log-in to Posit Cloud
<!-- add instructions on how to make the project use collaborative editing -->
- Open the `PA3-dplyr.qmd` file (from the lower right corner)
- Get started! 
