...
::: callout-note
`str_c()` with `sep = ""` will do the same thing, but I would recommending using `str_flatten()` instead!
:::
...
- `stringr` functions for different tasks
- Regular expressions for different tasks
:::
:::
:::
...
- Open the PA 5: Decode Message project
- Open the `PA5-stringr.qmd` file
:::

. . .

::: {.small}
Then, the partner who *currently* has the most pets starts as the Developer (typing
...
- Each person will input the full name of the movie the scrambled message is from. 
- The person who last occupied the role of Developer will download and submit
the `PA-5.html` file for the group.
