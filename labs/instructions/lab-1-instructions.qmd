...
Use the steps below to make a copy of the Lab 1 repository: [Copying the Lab Assignment with GitHub Classroom](https://scribehow.com/shared/Copying_the_Lab_Assignment_with_GitHub_Classroom__jKUfxzJUQjOQ2lqJy2UPoQ) 

<!-- You can find the Lab 1 materials in the [Lab 1 repository on GitHub](https://github.com/atheobold/stat-331-lab-1). When you click on the provided link, you will end up on the landing page for the repository. -->

<!-- To make a copy of the project you need to carry out the following steps: -->

<!-- 1.  Click on icon that says ![](images/fork.png){width="15%"}, to create your own copy of **my** repository -->
...
11. Navigate to the "Lab 1" folder you made in the Week 1 folder of your STAT 331 folder. Once you get there, click "Open".
...
-   In the upper right corner you should see a blue R cube with `stat-331-lab-1` written next to it ![](images/new-r-project.png){width="15%"}, this tells you that you are working in an R Project (we'll learn more about these next week).
...
To https://github.com/atheobold/stat-331-lab-1.git
...
## Step 5: Let's get started working with Quarto!

In the Files pane, click on `lab-1.qmd` to open the Quarto file you will be working in for Lab 1.
...
*`ToothGrowth` is a data frame with \_\_\_\_ observations and \_\_\_\_ variables.*
...
ToothGrowth %>% 
  ...
  ggplot(mapping = aes(x = as.factor(dose), y = len)) +
  geom_boxplot() +
...
1.  Type <kbd>ctrl</kbd> + <kbd>alt</kbd> + <kbd>i</kbd> (on a PC) or <kbd>⌘</kbd> + <kbd>⌥</kbd> + <kbd>i</kbd> (on a Mac).

2.  Click on the ![](images/insert_code.png){width="5%"} symbol. This should automatically default to R code, but if you have a Python compiler on your computer, you might need to select "R" from the options.
...
Another common issue is deleting the tick marks (```` ``` ````) that surround your code chunks. If you notice that the code chunks are not showing a "Play" button (![](images/code-play-button.png){width="5%"}), or that they are not highlighted in gray, double check your tick marks!
