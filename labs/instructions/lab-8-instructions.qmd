...
iterate over certain tasks. 

<!-- To do this, we will need to load in the data from  -->
<!-- Lab 2, Lab 3, and Lab 7. I've included **all** the datasets in the `data` -->
<!-- folder, so all you need to do is read them in. 🙃  -->

```{r}
#| label: read-in-old-datad

# Data from Lab 2
surveys <- read_csv(here::here("labs", "instructions", "data", "surveys.csv"))

# Data from Lab 3
evals <- read_csv(here::here("labs", "instructions", "data", "teacher_evals.csv")) |> 
  rename(sex = gender)
...
**2. Using `map_at()`, convert the `teacher_id`, `weekday`, `academic_degree`, `seniority`, and `sex` columns to factors.** Hint: You will need to use `bind_cols()` to transform the 
